...
#else
	struct work_struct usbc_dp_work;
...
#if IS_ENABLED(CONFIG_SND_SOC_FSA_DP)
static int fsa4480_mux_set(struct fsa4480_priv *fsa_priv);
#endif
				      ...
				      unsigned long evt, void *ptr)
{
	...
	if (fsa_priv->vendor == DIO4483) {
		dev_info(dev, "%s: switch chip is DIO4483\n", __func__);
	}
...
#if IS_ENABLED(CONFIG_SND_SOC_FSA_DP)
		dev_info(dev, "%s: old_state: %d, new_state: %d dp stat: %d\n",
			__func__, noti->typec_state.old_state, noti->typec_state.new_state, fsa_priv->is_dp);
		if ((noti->typec_state.old_state == TYPEC_ATTACHED_SRC
			|| noti->typec_state.old_state == TYPEC_ATTACHED_SNK)
			&& (noti->typec_state.new_state == TYPEC_UNATTACHED) && fsa_priv->is_dp) {
			fsa_priv->is_dp = false;
			dev_info(dev, "%s: DP plug out\n", __func__);
			pm_stay_awake(fsa_priv->dev);
			cancel_work_sync(&fsa_priv->usbc_dp_work);
			schedule_work(&fsa_priv->usbc_dp_work);
	...
	case TCP_NOTIFY_AMA_DP_STATE:
		dev_info(dev, "%s: DP plug in\n", __func__);
		dev_info(dev, "%s sel_config = %u, signal = %u\n",
				    __func__, noti->ama_dp_state.sel_config,
				    noti->ama_dp_state.signal);
		dev_info(dev, "%s pin_assignment = %u, polarity = %u\n",
				    __func__, noti->ama_dp_state.pin_assignment,
				    noti->ama_dp_state.polarity);
		dev_info(dev, "%s active = %u\n",
				    __func__, noti->ama_dp_state.active);
		if (fsa_priv->is_dp) {
			dev_info(dev, "%s: DP already in\n", __func__);
			return NOTIFY_OK;
		}
		fsa_priv->orientation = noti->ama_dp_state.polarity;
		fsa_priv->is_dp = true;
		pm_stay_awake(fsa_priv->dev);
		cancel_work_sync(&fsa_priv->usbc_dp_work);
		schedule_work(&fsa_priv->usbc_dp_work);
...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	if (fsa_priv->plug_state) {
		...
		if ((jack_status & 0x2)&&(fsa_priv->vendor != DIO4480)&&(fsa_priv->vendor != DIO4483)) {
...
#if IS_ENABLED(CONFIG_SND_SOC_FSA_DP)
static void fsa4480_usbc_dp_work_fn(struct work_struct *work)
{
	struct fsa4480_priv *fsa_priv =
		container_of(work, struct fsa4480_priv, usbc_dp_work);

	if (!fsa_priv) {
		pr_info("%s: fsa container invalid\n", __func__);
		return;
	}
	fsa4480_mux_set(fsa_priv);
	pm_relax(fsa_priv->dev);
}
#endif
...
static int fsa4480_mux_set(struct fsa4480_priv *fsa_priv)
{
	...
	mutex_lock(&fsa_priv->notification_lock);
	if (fsa_priv->is_dp) {
		switch (fsa_priv->orientation) {
		case USB_TYPEC_NORMAL:
			fsa4480_usbc_update_settings(fsa_priv, 0x18, 0xF8);
			ret = fsa4480_validate_display_port_settings(fsa_priv);
			break;
...
#endif
{
	...
	if (HL5280_DEVICE_REG_VALUE == reg_value) {
        ...
        } else if (DIO4483_DEVICE_REG_VALUE == reg_value) {
	...
	if ((fsa_priv->vendor == DIO4480) || (fsa_priv->vendor == WAS4780) || (fsa_priv->vendor == DIO4483)) {
		regmap_write(fsa_priv->regmap, 0x1e, 0x01);//reset DIO4480
		usleep_range(1*1000, 1*1005);
	}

	if (fsa_priv->vendor == DIO4483) {
		regmap_write(fsa_priv->regmap, 0x1c, 0x2a);
		regmap_write(fsa_priv->regmap, 0x1f, 0x0f);
		regmap_write(fsa_priv->regmap, 0x2e, 0x8f);
		regmap_write(fsa_priv->regmap, 0x2f, 0x44);
		regmap_write(fsa_priv->regmap, 0x2e, 0x72);
		usleep_range(1*1000, 1*1005);
		regmap_write(fsa_priv->regmap, 0x0f, 0x00);
		regmap_write(fsa_priv->regmap, 0x10, 0x00);
		regmap_write(fsa_priv->regmap, 0x0e, 0x0b);
		regmap_write(fsa_priv->regmap, 0x0d, 0x0f);
		regmap_write(fsa_priv->regmap, 0x21, 0x0f);
		usleep_range(1*1000, 1*1005);
	}
...
#else
	fsa_priv->orientation = TYPEC_ORIENTATION_NONE;
	fsa_priv->is_dp = false;
	INIT_WORK(&fsa_priv->usbc_dp_work, fsa4480_usbc_dp_work_fn);
#endif
	/* add end for DP part */
