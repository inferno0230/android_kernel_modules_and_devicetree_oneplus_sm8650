...
enum aw87xxx_scene_mode {
        AW87XXX_MUSIC_MODE = 0,
        AW87XXX_VOICE_MODE = 1,
        AW87XXX_FM_MODE = 2,
        AW87XXX_RCV_MODE = 3,
        AW87XXX_OFF_MODE = 4,
        AW87XXX_MAX_MODE,
...
static char *aw87xxx_profile[AW87XXX_MAX_MODE] = {
        [AW87XXX_MUSIC_MODE] = "Music",
        [AW87XXX_VOICE_MODE] = "Voice",
        [AW87XXX_FM_MODE] = "Fm",
        [AW87XXX_RCV_MODE] = "Receiver",
        [AW87XXX_OFF_MODE] = "Off",
};

void *oplus_pa_aw_2_x_0_node = NULL;
uint8_t aw_pa_mode_2_x_0 = AW87XXX_MUSIC_MODE;
#endif /* CONFIG_SND_SOC_OPLUS_PA_MANAGER */
...
int aw87xxx_update_profile(struct aw87xxx *aw87xxx, char *profile)
{
	...
	aw87xxx_2_x_0_monitor_stop(&aw87xxx->monitor);
	if (strncmp(profile, aw87xxx->prof_off_name, AW_PROFILE_STR_MAX) == 0) {
		...
		if (!ret)
			aw87xxx_2_x_0_monitor_start(&aw87xxx->monitor);
	}
	mutex_unlock(&aw87xxx->reg_lock);
...
void aw87xxx_2_x_0_enable_pa(int enable, int mode, int32_t channel) {
        if(enable) {
                if(mode == SPK_MODE) {
                        aw87xxx_set_profile(channel, aw87xxx_profile[aw_pa_mode_2_x_0]);
                } else if (mode == RECV_MODE) {
                        aw87xxx_set_profile(channel, aw87xxx_profile[AW87XXX_RCV_MODE]);
                }
        } else {
                aw87xxx_set_profile(channel, aw87xxx_profile[AW87XXX_OFF_MODE]);
        }
}

void aw87xxx_2_x_0_enable_l_pa(int enable, int mode){
        aw87xxx_2_x_0_enable_pa(enable, mode, AW87XXX_LEFT_CHANNEL);
}
...
int ext_2_x_0_amp_low_voltage_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
...
int ext_2_x_0_amp_low_voltage_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.integer.value[0] = aw87xxx_2_x_0_spk_low_voltage_status;
        pr_info("%s: aw87xxx_2_x_0_spk_low_voltage_status = %d\n", __func__, aw87xxx_2_x_0_spk_low_voltage_status);
		pr_info("%s\n", __func__);
        return 0;
}

int aw87xxx_2_x_0_spk_mode_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
	...
	if(ucontrol->value.integer.value[0] == 0){
		aw_pa_mode_2_x_0 = AW87XXX_MUSIC_MODE;
	}else if(ucontrol->value.integer.value[0] == 1){
		aw_pa_mode_2_x_0 = AW87XXX_VOICE_MODE;
	}else if(ucontrol->value.integer.value[0] == 2){
		aw_pa_mode_2_x_0 = AW87XXX_FM_MODE;
	}

	pr_info("%s() = %d\n", __func__, aw_pa_mode_2_x_0);

	return 0;
}

int aw87xxx_2_x_0_spk_mode_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol){
	pr_info("%s() = %d\n", __func__, aw_pa_mode_2_x_0);
	ucontrol->value.integer.value[0] = aw_pa_mode_2_x_0;

	return 0;
}


bool is_mute_2_x_0_status = false;
int aw87xxx_2_x_0_speaker_mute_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
...
int aw87xxx_2_x_0_speaker_mute_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s() = %d\n", __func__, is_mute_2_x_0_status);
	ucontrol->value.integer.value[0] = is_mute_2_x_0_status;
	return 0;
}
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
				...
				struct device_node *dev_node)
{
	...
	if (ret < 0) {
		AW_DEV_LOGI(aw87xxx->dev, "low_volume_voltage parse failed, user default[%d], ret=%d",
				AW87XXX_LOW_VOLUME_VOLTAGE, ret);
		aw87xxx->low_volume_voltage = AW87XXX_LOW_VOLUME_VOLTAGE;
	} else {
		aw87xxx->low_volume_voltage = low_volume_voltage;
		AW_DEV_LOGI(aw87xxx->dev, "parse low_volume_voltage=[%d]",
				aw87xxx->low_volume_voltage);
	}
...
#endif
{
	...
	ret = of_property_read_u32(dev_node, "oplus-real-addr", &real_addr);
	...
	/*monitor init*/
	aw87xxx_2_x_0_monitor_init(aw87xxx->dev, &aw87xxx->monitor, dev_node);
	...
	if ((speaker_device !=NULL) && (aw87xxx->dev_index == 0)) {
		speaker_device->chipset = MFR_AWINIC;
		speaker_device->type = L_SPK;
		speaker_device->vdd_need = 0;
		speaker_device->speaker_enable_set = aw87xxx_2_x_0_enable_l_pa;
	...
	} else if ((speaker_device !=NULL) && (aw87xxx->dev_index == 1)) {
		speaker_device->chipset = MFR_AWINIC;
		speaker_device->type = R_SPK;
		speaker_device->vdd_need = 0;
		speaker_device->speaker_enable_set = aw87xxx_2_x_0_enable_r_pa;
		speaker_device->boost_voltage_set = ext_2_x_0_amp_low_voltage_set;
		speaker_device->boost_voltage_get = ext_2_x_0_amp_low_voltage_get;
		speaker_device->spk_mode_set = aw87xxx_2_x_0_spk_mode_set;
		speaker_device->spk_mode_get = aw87xxx_2_x_0_spk_mode_get;
	...
	if (new_speaker_device_node == true) {
		...
		oplus_pa_aw_2_x_0_node = oplus_speaker_pa_register(speaker_device);
	}
...
exit_device_init_failed:
	...
	if (gpio_is_valid(aw87xxx->aw_dev.rst_gpio)) {
		...
		devm_gpiod_put(&client->dev, desc);
	}
...
#if IS_ENABLED(CONFIG_SND_SOC_OPLUS_PA_MANAGER)
	oplus_speaker_pa_remove(oplus_pa_aw_2_x_0_node);
#endif /*CONFIG_SND_SOC_OPLUS_PA_MANAGER*/
	if (aw87xxx->dev_index == 0)
