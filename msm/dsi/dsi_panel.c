...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	iris_reset(); /* Move to dsi_panel_prepare */
#endif
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if(panel->bl_ic_ktz8866_used) {
		...
		rc = bl_ic_ktz8866_set_lcd_bias_by_gpio(true);
		if (rc) {
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_EXTERNAL:
		...
		if(panel->bl_ic_ktz8866_used) {
			rc = bl_ic_ktz8866_set_brightness(bl_lvl);
		}
		#endif
		break;
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-pwm-switch-onepulse-low-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-pwm-switch-onepulse-low-command-state",
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	panel->bl_ic_ktz8866_used = utils->read_bool(utils->data, "oplus,bl-use-ktz8866-ic-ctrl");
	LCD_INFO("oplus,bl-use-ktz8866-ic-ctrl: %s\n", panel->bl_ic_ktz8866_used ? "true" : "false");
#endif /* OPLUS_FEATURE_DISPLAY */
		...
		struct dsi_parser_utils *utils)
{
	...
	if (mode->panel_mode_caps & DSI_OP_VIDEO_MODE) {
		...
		DSI_INFO("h_total_dce=%u, v_total=%u, refresh_rate=%u, pclk = %u, h_total=%u \n",
...
static int oplus_power_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	DSI_INFO("notifier event:%d, pon_status:%d bl_lvl_backup:%d\n", event, evdata->pon_status, bl_lvl_backup);
				...
				bool trusted_vm_env)
{
	...
	if (is_project(22111) || is_project(22112)) {
		if (!strcmp(panel->name, "AA545 P 3 A0005 dsc cmd mode panel")) {
			panel->name = "AC090 P 3 A0005 dsc cmd mode panel";
		}
		if (!strcmp(panel->name, "AA545 P 1 A0006 dsc cmd mode panel")) {
			panel->name = "AC090 P 1 A0006 dsc cmd mode panel";
		}
	}
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
	if (!strcmp(panel->name, "Dual dsi csot nt36532 video mode panel with DSC") \
		...
		if ((tp_gesture_enable_notifier && (1 != tp_gesture_enable_notifier(0))) || (1 == shutdown_flag)) {
			...
			OPLUS_PANEL_EVENT_NOTIFY(0, DRM_PANEL_EVENT_FOR_TOUCH, 0x1A);
			//if (gpio_is_valid(panel->reset_config.reset_gpio))
			//	gpio_set_value(panel->reset_config.reset_gpio, 0);
		}
	}
